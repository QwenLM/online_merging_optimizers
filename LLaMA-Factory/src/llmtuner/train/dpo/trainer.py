...
class CustomDPOTrainer(DPOTrainer):
    def __init__(
        ...
        base_model: Optional[Union["PreTrainedModel", torch.nn.Module]],
        ...
        **kwargs,
        ...
        if finetuning_args.online_step != 1:
            print('Save only model with online step larger than 1:', finetuning_args.online_step)
            self.args.save_only_model = True

    def create_optimizer(self) -> "torch.optim.Optimizer":
        if self.optimizer is None:
            self.optimizer = create_custom_optimzer(self.model, self.args, self.finetuning_args, base_model=self.base_model, ref_model=self.ref_model)
