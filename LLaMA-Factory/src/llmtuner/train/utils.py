...
def create_ref_model(
    ...
    if finetuning_args.ref_model is not None:
        ...
        logger.info("Begin Created reference model from {}".format(finetuning_args.ref_model))
        ref_model = load_model(
            tokenizer, ref_model_args, ref_finetuning_args, is_trainable=False, add_valuehead=add_valuehead
        )
        logger.info("End Created reference model from {}".format(finetuning_args.ref_model))
    else:
...
def create_base_model(
    ...
    The valuehead parameter is randomly initialized since it is useless for PPO training.
    """
    if finetuning_args.base_model is not None:
        base_model_args_dict = model_args.to_dict()
        base_model_args_dict.update(
            dict(
                model_name_or_path=finetuning_args.base_model,
                adapter_name_or_path=finetuning_args.base_model_adapters,
                quantization_bit=finetuning_args.base_model_quantization_bit,
            )
        )
        base_model_args = ModelArguments(**base_model_args_dict)
        base_finetuning_args = FinetuningArguments(finetuning_type="lora")
        tokenizer = load_tokenizer(base_model_args)["tokenizer"]
        logger.info("Begin Created base model from {}".format(finetuning_args.ref_model))
        base_model = load_model(
            tokenizer, base_model_args, base_finetuning_args, is_trainable=False, add_valuehead=add_valuehead
        )
        logger.info("End Created base model from {}".format(finetuning_args.base_model))
    else:
        if finetuning_args.finetuning_type == "lora":
            base_model = None
        else:
            ...
            logger.info("Created base model from the model itself.")
...
def _create_online_merging_optimizer(
    ...
    from .optimizers import ODAdamW, OTAdamW
    if finetuning_args.om_mode == 'ondare':
        optimizer_cls = ODAdamW
    elif finetuning_args.om_mode == 'onties':
...
def _create_ema_optimizer(
    model: "PreTrainedModel",
    training_args: "Seq2SeqTrainingArguments",
    finetuning_args: "FinetuningArguments",
) -> "torch.optim.Optimizer":
    from .optimizers import EMAAdamW
    optimizer_cls = EMAAdamW
    ...
    optimizer_kwargs.update({
        "ema_rate": finetuning_args.ema_rate
    })
    optimizer = optimizer_cls(optimizer_grouped_parameters, **optimizer_kwargs)
    return optimizer
...
def create_custom_optimzer(
    ...
    if finetuning_args.use_galore and not finetuning_args.use_online_merging:
    ...
    if finetuning_args.use_online_merging:
        return _create_online_merging_optimizer(model, training_args, finetuning_args, base_model, ref_model)
    
    if finetuning_args.use_ema:
        return _create_ema_optimizer(model, training_args, finetuning_args)
